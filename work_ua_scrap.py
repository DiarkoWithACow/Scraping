...
def workua_scraping(driver):
    ...
    for page_count in range(1, 4):
        ...
        for vacancy in vacancies.element_handles():
            ...
            company_name = vacancy.query_selector("div > span.strong-600")
            if company_name:
            ...
            salaryinfo  = vacancy.query_selector(".mt-xs .strong-600")
            if salaryinfo:
            ...
            print(f"  ✅ {item['Title']} | {item['Company_Name']}  | {item['Location']} | {item['Salary_info']}")
            jobs.append(item)
